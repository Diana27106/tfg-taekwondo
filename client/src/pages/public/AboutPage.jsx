...
const AboutPage = () => {
  ...
  return (
    <div className="relative w-full min-h-screen font-sans overflow-hidden">
      ...
      {/* SECCIÓN 1: HISTORIA DE LA ESCUELA */}
      <div className="bg-white p-8 md:p-16 flex flex-col md:flex-row items-stretch gap-10 font-sans mt-20">
        {/* Contenedor de la Imagen */}
        {/* Agregamos min-h-[300px] para que en móvil tenga una altura base. En PC tomará la altura del texto. */}
        <div className="w-full md:w-1/2 relative min-h-[300px] flex-shrink-0">
          <div className="absolute top-[-47px] bottom-[-47px] md:top-[-79px] md:bottom-[-79px] left-0 right-0">

            <img
              src="../../../src/assets/img/medium/historia.jpg" // Reemplaza con la ruta de tu imagen
              alt="Maestro de Taekwon-Do"
              className="shadow-xl grayscale object-cover w-full h-full"
            />
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 text-gray-800 flex flex-col justify-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-6 tracking-tight" style={{ fontFamily: 'Orbitron, sans-serif' }}>
      ...
      <div className="relative w-full mt-24 pb-24">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-8 relative z-10">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between mb-32 min-h-[550px] gap-12 lg:gap-8">
            ...
            {/* Izquierda: Etiquetas flotantes de los principios (Mosaico en Cascada) */}
            <div className="w-full lg:w-1/2 relative flex flex-col justify-center min-h-[400px] lg:min-h-[550px] gap-5 py-10 lg:pr-10">
              <div
                onClick={() => setIsCortesiaOpen(true)}
                className="self-start lg:ml-0 bg-white/90 backdrop-blur-md border border-orange-200 text-orange-600 rounded-2xl px-10 py-4 shadow-xl font-bold text-xl tracking-wide hover:-translate-y-1 hover:scale-105 hover:shadow-orange-500/20 transition-all duration-300 cursor-pointer"
              >
                Cortesía
              </div>
              <div
                onClick={() => setIsIntegridadOpen(true)}
                className="self-start ml-6 sm:ml-12 lg:ml-14 bg-white/90 backdrop-blur-md border border-green-200 text-green-700 rounded-2xl px-10 py-4 shadow-xl font-bold text-xl tracking-wide hover:-translate-y-1 hover:scale-105 hover:shadow-green-500/20 transition-all duration-300 cursor-pointer"
              >
                Integridad
              </div>
              <div
                onClick={() => setIsPerseveranciaOpen(true)}
                className="self-start ml-12 sm:ml-24 lg:ml-28 bg-white/90 backdrop-blur-md border border-blue-200 text-blue-600 rounded-2xl px-10 py-4 shadow-xl font-bold text-xl tracking-wide hover:-translate-y-1 hover:scale-105 hover:shadow-blue-500/20 transition-all duration-300 cursor-pointer"
              >
                Perseverancia
              </div>
              <div
                onClick={() => setIsAutocontrolOpen(true)}
                className="self-start ml-18 sm:ml-36 lg:ml-40 bg-white/90 backdrop-blur-md border border-red-200 text-red-600 rounded-2xl px-10 py-4 shadow-xl font-bold text-xl tracking-wide hover:-translate-y-1 hover:scale-105 hover:shadow-red-500/20 transition-all duration-300 cursor-pointer"
              >
                Autocontrol
              </div>
              <div
                onClick={() => setIsEspirituOpen(true)}
                className="self-start ml-24 sm:ml-48 lg:ml-52 bg-white/90 backdrop-blur-md border border-slate-300 text-slate-800 rounded-2xl px-10 py-4 shadow-xl font-bold text-xl tracking-wide hover:-translate-y-1 hover:scale-105 hover:shadow-slate-500/20 transition-all duration-300 cursor-pointer"
              >
                Espíritu Indomable
              </div>
            </div>
          ...
          <div className="mt-20">
            ...
            {!loading && !error && instructors.length > 0 && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-24 pt-8">
